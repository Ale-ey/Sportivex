...
import type { ContentSection } from "@/constant/data/blogPosts";
import {
  DollarSign,
  Users,
  TrendingUp,
    ...
    switch (section.type) {
      case "heading":
        return (
          <h2
            key={index}
            className="text-3xl font-bold mb-6 text-black border-b-2 border-black pb-2"
          >
            {section.content}
          </h2>
        );
          ...
          <div key={index} className="mb-8">
            <ol className="list-none space-y-3">
              {section.items?.map((item, itemIndex) => (
                <li
                  key={itemIndex}
                  className="text-gray-800 leading-relaxed text-sm bg-gray-50 p-3 rounded border-l-4 border-black"
                >
                  {item}
                </li>
              ))}
      ...
      case "table":
        ...
        return (
          <Card
            key={index}
            className="border-2 border-black mb-8 overflow-hidden"
          >
      ...
      case "quote":
        return (
          <Card
            key={index}
            className="border-2 border-black bg-black text-white mb-8"
          >
